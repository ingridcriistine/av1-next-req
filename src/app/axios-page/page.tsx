...
const AxiosPage = () => {
    ...
    return(
        <div className="bg-gray-900 text-white">
            <input type="number" value={page} onChange={(e) => setPage(e.target.value)} placeholder="1/149 - insira a página" className="text-black m-4 pl-2 mt-32"/>
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Pesquise um nome" className="text-black m-4 pl-2 mt-32"/>
            <h1 className="font-bold p-4 pt-16 flex justify-center text-[22px]">Personagens</h1>
                ...
                {data.map((item, index) => {
                    return(
                        <div key={item._id} className="bg-white rounded m-4 text-black w-[250px] flex flex-col items-center justify-center">
                            ...
                            <Image className="w-[200px] h-[220px] mb-6 rounded" src={item.imageUrl} width={1000} height={1000} alt="imagem" priority/>
                        </div>
                    )
